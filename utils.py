...
# recalib = recalibrator.adjust(torch.linspace(0, 1, 500)).cpu() 
# plt.plot(torch.linspace(0, 1, 500), recalib)
# plt.show()

# with torch.no_grad():
#     inputs, labels = test_dataset[:][0].to(args.device)[200:], test_dataset[:][1].to(args.device)[200:]
#     outputs = model(inputs)
#     plt.hist(outputs.flatten().cpu().numpy(), bins=30, alpha=0.5, color='r')
#     plt.hist(recalibrator.adjust(outputs).flatten().cpu().numpy(), bins=30, alpha=0.5, color='b')
#     plt.hist(labels.flatten().cpu().numpy(), bins=30, alpha=0.5, color='g')
