...
for runs in range(args.num_run):
    while True:
        args.name = '%s%s/tanh_model=%s-%r-%r-%r-%r-bs=%d-run=%d' % \
    ...
    global_iteration = 0
    random.seed(args.run_label * args.seed)  # Set a different random seed for different run labels
    ...
    model = model_list[args.model](train_dataset.x_dim, act=F.tanh).to(device) #F.leaky_relu remove act
